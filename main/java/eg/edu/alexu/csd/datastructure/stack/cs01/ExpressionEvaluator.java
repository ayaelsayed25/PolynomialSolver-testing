        ...
        for (int i = 0; i < size; i++) {
            if (!isSymbolOrOperator(str[i])) {
                ...
                if (!str[i].equals("-") && Pattern.matches("^[-]\\d*$", str[i])) {
                    exp.add(new String("(")); exp.add(new String("0")); exp.add(new String("-"));
                    int s = Integer.parseInt(str[i]);
                    s = -1 * s;
                    String myS = Integer.toString(s);
                    exp.add(new String(myS)); exp.add(new String(")"));
                }
                else if(str[i].equals("-("))
                {
                    exp.add(new String("(")); exp.add(new String("0"));exp.add(new String("-"));exp.add(new String("1"));exp.add(new String(")"));
                    exp.add(new String("*")); exp.add(new String("(")); 
                }
                else {
